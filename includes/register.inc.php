...
if (isset($_POST['teamname'], $_POST['email'], $_POST['p'],$_POST['mobile'],$_POST['age'],$_POST['gender'])) {
   ...
   // check existing email  
    ...
    $prep_stmt = "SELECT id FROM volunteer WHERE email = ? LIMIT 1";
    $stmt = $mysqli->prepare($prep_stmt);
...
elseif (isset($_POST['volunteer'], $_POST['email'], $_POST['p'],$_POST['mobile'],$_POST['age'],$_POST['gender'],$_POST['city'])) {
   ...
   // check existing email  
    ...
    // check existing teamname
    $prep_stmt = "SELECT id FROM volunteer WHERE volunteer = ? LIMIT 1";
    $stmt = $mysqli->prepare($prep_stmt);
    ...
    if (empty($error_msg)) {
        ...
        if ($insert_stmt = $mysqli->prepare("INSERT into volunteer SET volunteer='$volunteer', email='$email', password ='$password' ,mobile ='$mobile', age='$age', gender='$gender',city='$city'")) {
            ...
            if ( $insert_stmt->execute()) {
                ...
                $insert_stmt1 = $mysqli->prepare("UPDATE volunteer SET flag='0' WHERE volunteer = '$volunteer' ");
                $insert_stmt1->execute();
                header('Location: ./instruction.php');
            }
